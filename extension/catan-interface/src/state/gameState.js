 ...
 * captures — see the repo NOTES.md §3.
 *
 * Protocol-shaped but not protocol-DECODING (that's src/protocol/). No DOM, no external imports.
 * The clock is injectable (opts.now) so tests can pass a deterministic time source.
 */
...
export class GameState {
  ...
  _emit(evt) { this.rev++; for (const fn of this._subs) { try { fn(this, evt); } catch (e) { safeWarn(e); } } }
  ...
  // ---- convenience accessors (used by HUD + renderer + observation) ----
  ...
  // ---- logging ----
  _logEvent(kind, data) {
    this.log.push({ rev: this.rev, kind, data, t: this._now() });
