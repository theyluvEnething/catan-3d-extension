 ...
 */
export function attachWatchdog(state, { onDesync } = {}) {
  ...
  state._applySnapshot = function (payload) {
    ...
    if (state.ready && priorBoard && incoming) {
      ...
      if (drifts.length) {
        desyncs++;
        const rec = { t: nowSafe(), drifts };
        history.push(rec);
        if (history.length > 50) history.shift();
        for (const cb of cbs) { try { cb(drifts, rec); } catch {} }
        try { console.warn("[catan3d/watchdog] DESYNC detected —", drifts.length, "field(s):", JSON.stringify(drifts.slice(0, 6))); } catch {}
      }
    }
