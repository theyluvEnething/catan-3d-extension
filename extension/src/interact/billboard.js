...
export class ConfirmBillboard {
  ...
  _build() {
    ...
    el.style.cssText = "position:absolute;left:0;top:0;transform:translate(-50%,-100%);z-index:20;pointer-events:auto;display:none;cursor:pointer;filter:drop-shadow(0 6px 12px rgba(0,0,0,.45));will-change:left,top;";
    el.innerHTML = `
      <div class="bb-plate" style="position:relative;width:76px;height:76px;background-size:100% 100%;display:grid;place-items:center;">
        <img class="bb-icon" alt="" style="width:48px;height:48px;pointer-events:none;">
        <span class="bb-check" style="position:absolute;right:-6px;bottom:-6px;width:26px;height:26px;border-radius:50%;background:#3ec85a;display:grid;place-items:center;box-shadow:0 2px 6px rgba(0,0,0,.4);">
      ...
      <div class="bb-stem" style="position:absolute;left:50%;bottom:-10px;transform:translateX(-50%);width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:11px solid #cfe6f2;"></div>`;
  ...
  _tick() {
    ...
    // project() gives NDC in [-1,1]; map to the overlay's local pixel space.
    const hostRect = this.host.getBoundingClientRect();
    const x = (this._world.x * 0.5 + 0.5) * rect.width + (rect.left - hostRect.left);
    const y = (-this._world.y * 0.5 + 0.5) * rect.height + (rect.top - hostRect.top);
